Feature: Basic File Reading and Validation
  ...
  Scenario: Read and validate a simple text file
    Given the user have a text file "src/test/resources/testdata/sample.txt" with content:
      """
      username=admin
      password=secret123
      environment=staging
  ...
  Scenario: Verify file existence and properties
    Given the user have a file "src/test/resources/testdata/config.txt"
  ...
  Scenario: Handle missing file gracefully
    Given the user have file path "src/test/resources/testdata/nonexistent.txt"
  ...
  Scenario: Read file and parse key-value pairs
    Given the user have properties file "src/test/resources/testdata/app.properties" with content:
      """
      app.name=TestApp
      app.version=1.0.0
      app.port=8080
